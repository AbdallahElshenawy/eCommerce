...
{
    public class UserManagement(IRoleManagement roleManager,UserManager<AppUser> userManager,AppDbContext context) : IUserManagement
    {
        ...
        public async Task<List<Claim>> GetUserClaims(string email)
        {
            ...
            string? roleName = await roleManager.GetUserRole(user!.Email!);
            List<Claim> claims = [
